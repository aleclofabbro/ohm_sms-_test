...
export function getPipeline(query: string): AnyObject[] {
  ...
  const currentPathBlocks: (string|string[])[] = []
  const semantics = grammar.createSemantics()
  .addOperation('gm()', {
    SetOp(_SetOp, dotPath, _colon, jsonString) {
      const json = jsonString.gm()
      return {  [dotPath.sourceString]: json } 
    },
    // AddOp(_AddOp, b, c) {
    // },
    ...
    Query(target, statements) {
      const block = target.gm()
      
      statements.children.map((_) => _.gm())
    },
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    Target(_ON, target, _par1, ids, _par2) {
      const nextPathSegment = target.sourceString.split('.')
      const currentPath = currentPathBlocks.flat()
      const inIdsArr = getInIdsArr()
      const block = generateRecursiveArrayMap({currentPath,nextPathSegment,leafExpression:{
        $cond:{
          if: { '$$el.id': { $in: inIdsArr } },
          then: {},
          else: {}
        }
      }})

      currentPathBlocks.push(nextPathSegment)
